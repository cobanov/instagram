...
/* The Chrome extension used to carry a hand-copied version of this script,
   ...
   the same stripped output makes drift impossible. It is written without a
   header comment on purpose: byte-identical to dist means the SHA-256 the
   security page advertises verifies the extension copy too. The "do not edit
   by hand" warning lives in chrome-extension/README.md instead. */
...
fs.writeFileSync(extensionScriptPath, output, "utf8");
...
/* The extension ships the same script, so it carries the same version number.
   Chrome reads manifest.json and nothing else, so the check belongs here. */
const manifestPath = path.join(extensionDir, "manifest.json");
const manifestVersion = JSON.parse(fs.readFileSync(manifestPath, "utf8")).version;
if (manifestVersion !== entries[0].version) {
  throw new Error(
    `chrome-extension/manifest.json is ${manifestVersion} but changelog.json ships ${entries[0].version}`
  );
}
