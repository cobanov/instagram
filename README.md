...
when you click its toolbar icon, so there is no console step. Download the zip
...
Its copy of the script is generated by `npm run build` and is byte for byte the
same as `dist/`, so the hash on the security page verifies it too. See
[chrome-extension/README.md](chrome-extension/README.md).

Contributed by [yavuzmeteafsar](https://github.com/yavuzmeteafsar).
